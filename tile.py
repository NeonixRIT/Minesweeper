...
class Tile:
    def __init__(self, x: int, y: int, bomb: bool = False, hint: int = 0):
    ...
    def __str__(self) -> str:
        if self.is_flipped:
            hint = 5 if self.hint > 5 else self.hint
            return f'{Colors.RED.value}[●]{Colors.WHITE.value}' if self.is_bomb else f'{COLORS[hint]}[{self.hint}]{Colors.WHITE.value}' if self.hint > 0 else '[ ]'
        return '[X]'
